// Fedora Container Image with Sway Desktop
...
source "docker" "fedora_sway" {
  ...
  changes = [
    ...
    "LABEL description='Fedora image with Sway desktop environment'"
  ]
}
...
build {
  ...
  // Run Ansible provisioning with Sway desktop
  provisioner "ansible" {
    ...
    extra_arguments = [
      "--extra-vars",
      "ansible_connection=docker enable_k8s_tools=false enable_sway_desktop=true enable_virtualization=false enable_plymouth=false",
      "--tags",
      "base,security,sway,services"
    ]
